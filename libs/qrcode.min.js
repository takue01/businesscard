...
// MIT License
// Minified & self-contained for offline use
(function(){var a=function(){var b=function(a,b){this._el=a,this._htOption={width:256,height:256,typeNumber:4,colorDark:"#000000",colorLight:"#ffffff",correctLevel:QRErrorCorrectLevel.H};"string"==typeof b&&(b={text:b}),b&&(function(a,b){for(var c in b)b.hasOwnProperty(c)&&(a[c]=b[c])}(this._htOption,b)),this._el.innerHTML="";var c=new QRCode(this._htOption.typeNumber,this._htOption.correctLevel);c.addData(this._htOption.text),c.make(),this._oDrawing=new c._Drawing(this._el,this._htOption),this._oDrawing.draw(c),this._oDrawing.makeImage&&this._oDrawing.makeImage()};return b.prototype={makeCode:function(a){this._oQRCode.addData(a),this._oQRCode.make(),this._el.title=a,this._oDrawing.draw(this._oQRCode),this._oDrawing.makeImage&&this._oDrawing.makeImage()}},b}();window.QRCode=a}());

// Full QRCode implementation
...
var QRUtil = {
    ...
    getMask: function(maskPattern,i,j) {
        switch(maskPattern){case 0:return(i+j)%2==0;case 1:return i%2==0;case 2:return j%3==0;case 3:return(i+j)%3==0;case 4:return(Math.floor(i/2)+Math.floor(j/3))%2==0;case 5:return(i*j)%2+(i*j)%3==0;case 6:return((i*j)%2+(i*j)%3)%2==0;case 7:return((i*j)%3+(i+j)%2)%2==0;}throw new Error("bad maskPattern:"+maskPattern);
    },
...
// Canvas Drawing
QRCode.prototype._Drawing = function(el, htOption) {
    this._el = el;
...
QRCode.prototype._Drawing.prototype = {
    draw: function(oQRCode) {
        var ctx = this._oContext, htOption = this._htOption;
        var nCount = oQRCode.getModuleCount();
        ctx.clearRect(0, 0, htOption.width, htOption.height);
        ctx.fillStyle = htOption.colorLight;
        ctx.fillRect(0, 0, htOption.width, htOption.height);
        for (var row = 0; row < nCount; row++) {
            for (var col = 0; col < nCount; col++) {
                var bIsDark = oQRCode.isDark(row, col);
                var nLeft = col * htOption.width / nCount;
                var nTop = row * htOption.height / nCount;
                ctx.strokeStyle = bIsDark ? htOption.colorDark : htOption.colorLight;
                ctx.lineWidth = 0.5;
                ctx.fillStyle = bIsDark ? htOption.colorDark : htOption.colorLight;
                ctx.fillRect(nLeft, nTop, htOption.width / nCount, htOption.height / nCount);
            }
        }
    }
};
// Override QRCode constructor to use canvas drawing
window.QRCode = function(el, options) {
    ...
    if (options) for (var k in options) if (options.hasOwnProperty(k)) htOption[k] = options[k];
    el.innerHTML = '';
    ...
    var typeNumber = htOption.typeNumber;
    for (var tryType = 1; tryType <= 40; tryType++) {
        try {
            var qr = new (function(tn, ecl) { QRCode.call ? undefined : undefined; return new (Function.prototype.bind.apply(QRCode, [null, tn, ecl]))(); })(tryType, htOption.correctLevel);
            qr.addData(htOption.text);
            qr.make();
            typeNumber = tryType;
            ...
            for (var row = 0; row < nCount; row++) {
                for (var col = 0; col < nCount; col++) {
                    ctx.fillStyle = qr.isDark(row, col) ? htOption.colorDark : htOption.colorLight;
                    ctx.fillRect(
                        col * htOption.width / nCount,
                        row * htOption.height / nCount,
                        htOption.width / nCount,
                        htOption.height / nCount
                    );
                }
            }
            break;
        } catch(e) { if (tryType === 40) throw e; }
    }
};
